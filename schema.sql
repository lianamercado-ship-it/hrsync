...
CREATE TABLE empresas (
  ...
  sheet_id TEXT NOT NULL, catalogo_id TEXT NOT NULL,
  folder_id TEXT NOT NULL, activa BOOLEAN DEFAULT true,
...
CREATE TABLE usuario_empresa (
  ...
  puede_ejecutar BOOLEAN DEFAULT true, puede_generar BOOLEAN DEFAULT true,
...
CREATE TABLE reglas_empresa (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  empresa_id UUID REFERENCES empresas(id) ON DELETE CASCADE,
  clave TEXT NOT NULL, valor TEXT NOT NULL, descripcion TEXT,
...
CREATE TABLE ejecuciones (
  ...
  tipo TEXT NOT NULL, estado TEXT NOT NULL DEFAULT 'iniciado',
  filas_procesadas INTEGER, mensaje TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE periodos_pago (
  ...
  frecuencia TEXT DEFAULT 'B', nu_lote_he TEXT, nu_lote_fer TEXT,
  activo BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE POLICY "admin_empresas" ON empresas FOR ALL USING (
  EXISTS (SELECT 1 FROM perfiles WHERE id=auth.uid() AND rol='admin'));
CREATE POLICY "analista_empresas" ON empresas FOR SELECT USING (
